...
import ValidateEmsoApex  from '@salesforce/apex/EmsoValidationClass.getValidateEmsoOffline';

export default class VnosStudentaSt5 extends LightningElement {
    ...
    handleSubmit(event){
        ...
        ValidateEmsoApex({ emso: emso.value })
            .then(results => {
                if(results.length > 3){
                    emso.setCustomValidity(results);
                }
                else{
